...
public class IndexLifecycleTransitionTests extends ESTestCase {
    ...
    public void testMoveClusterStateToNextStep() {
        ...
        ClusterState newClusterState = IndexLifecycleTransition.moveClusterStateToNextStep(index, clusterState, currentStep, nextStep,
            () -> now, false);
        ...
        newClusterState = IndexLifecycleTransition.moveClusterStateToNextStep(index, clusterState, currentStep,
            nextStep, () -> now, false);
        assertClusterStateOnNextStep(clusterState, index, currentStep, nextStep, newClusterState, now);
    }
    ...
    public void testMoveClusterStateToNextStepSamePhase() {
        ...
        index = clusterState.metaData().index(indexName).getIndex();
        newClusterState = IndexLifecycleTransition.moveClusterStateToNextStep(index, clusterState, currentStep, nextStep,
            () -> now, false);
        assertClusterStateOnNextStep(clusterState, index, currentStep, nextStep, newClusterState, now);
    }
    ...
    public void testSuccessfulValidatedMoveClusterStateToNextStep() {
        ...
        PolicyStepsRegistry stepRegistry = IndexLifecycleRunnerTests.createOneStepPolicyStepRegistry(policyName, step, indexName);
        ...
        Index index = clusterState.metaData().index(indexName).getIndex();
        ClusterState newClusterState = IndexLifecycleTransition.moveClusterStateToStep(indexName, clusterState, currentStepKey,
    ...
    public void testValidatedMoveClusterStateToNextStepWithoutPolicy() {
        ...
        long now = randomNonNegativeLong();
        Step step = new MockStep(nextStepKey, nextStepKey);
        PolicyStepsRegistry stepRegistry = IndexLifecycleRunnerTests.createOneStepPolicyStepRegistry(policyName, step);
        ...
        IllegalArgumentException exception = expectThrows(IllegalArgumentException.class,
            () -> IndexLifecycleTransition.moveClusterStateToStep(indexName, clusterState, currentStepKey,
                nextStepKey, () -> now, stepRegistry));
    ...
    public void testValidatedMoveClusterStateToNextStepInvalidCurrentStep() {
        ...
        String policyName = "my_policy";
        Step.StepKey currentStepKey = new Step.StepKey("current_phase", "current_action", "current_step");
        Step.StepKey notCurrentStepKey = new Step.StepKey("not_current_phase", "not_current_action", "not_current_step");
        Step.StepKey nextStepKey = new Step.StepKey("next_phase", "next_action", "next_step");
        long now = randomNonNegativeLong();
        Step step = new MockStep(nextStepKey, nextStepKey);
        PolicyStepsRegistry stepRegistry = IndexLifecycleRunnerTests.createOneStepPolicyStepRegistry(policyName, step);
        ...
        ClusterState clusterState = buildClusterState(indexName, indexSettingsBuilder, lifecycleState.build(), Collections.emptyList());
        IllegalArgumentException exception = expectThrows(IllegalArgumentException.class,
            () -> IndexLifecycleTransition.moveClusterStateToStep(indexName, clusterState, notCurrentStepKey,
                nextStepKey, () -> now, stepRegistry));
    ...
    public void testMoveClusterStateToErrorStep() throws IOException {
        ...
        ClusterState newClusterState = IndexLifecycleTransition.moveClusterStateToErrorStep(index, clusterState, currentStep, cause,
        ...
        newClusterState = IndexLifecycleTransition.moveClusterStateToErrorStep(index, clusterState, currentStep, cause, () -> now,
    ...
    public void testValidateTransitionThrowsExceptionForMissingIndexPolicy() {
        ...
        PolicyStepsRegistry policyRegistry = IndexLifecycleRunnerTests.createOneStepPolicyStepRegistry("policy", currentStep);
    ...
    public void testValidateValidTransition() {
        ...
        PolicyStepsRegistry policyRegistry = IndexLifecycleRunnerTests.createOneStepPolicyStepRegistry(policy, finalStep);
